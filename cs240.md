...
**Best Case Analysis**: First chosen pivot could be the $k$th element. We have no recursive calls, so the total cost would be $\Theta(n)$.
...
- Recurrence relation is $T^{(avg)}(n) = T^{(avg)}(\sqrt n) + \Theta(1)$ which resolves to a runtime of $\Theta(\log(\log(n)))$.
...
- Main idea is that an item with key $k$ can only be in $T[h_1(k)]$ or $T[h_2(k)]$.
    ...
    - **Insert** always puts a new item into $T[h_1(k)]$. If it is already occupied, you *kick out* the other item, which we then attempt to reinsert into its alternate position.
        ...
        - In case of failure, we rehash with a larger $M$ and new hash functions.
...
- Try to choose hash-function that is unrelated to any possible patterns in the data and depends on all parts of the key.
...
- **Search**: as in a BST.
- **Insert**: first, insert point by x-coordinate in $T$. Then walk back up to the root and insert the point by y-coordinate in **all** $T(v)$ of nodes $v$ on path to the root.
...
- We follow with a block $b$ of 0s, where $|b|$ is one less than the number of bits required to hold the next block.
...
- To achieve even better compression, bzip2 uses **text transform**. We change the input into a different text that is not necessarily shorter, but has other desirable qualities.
