...
2. Allocation rule is **monotone** if for all $i \in N$ and $b_{-i} \in B_{-i}$, $x_i(z, b_{-i}) \ge x_i(y, b_{-i})$ whenever $z \ge y$.
...
> $(\Rightarrow)$ Assume $x$ is implementable. So there is payment rule $p$ such that $(x, p)$ is DISC. Fix player $i$, $b_{-i} \in B_{-i}$. Required to prove that $z \ge y$ implies $x_i(z, b_{-i}) \ge x_i(y, b_{-i})$. Use $x(y), p(y)$ to represent $x_i(y, b_{-i})$ and $p_i(y, b_{-i})$.
>
> $(x, p)$ is independent of player valuations, so DSIC for any possible valuation. So DSIC when player $i$'s valuation is $z$. So $u(z) \ge u(z)$, $z x(z) - p(z) \ge z z(y) - p(z)$. This also holds when player $i$'s valuation is $y$. So $y x(y) - p(y) \ge y x(z) - p(z)$.
>
> Combining inequalities we have $y(x(z) - x(y)) \le p(z) - p(y) \le z(x(z) - x(y))$. Since $y \le z$, $z, y \ge 0$, we know $x(z) - x(y) \ge 0$, so $x(z) \ge x(y)$. So $x$ is monotone.
>
> $(\Leftarrow)$.
